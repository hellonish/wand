...
function highlightText(
    ...
    const matches: Array<{ start: number; end: number; action: ActionItem; idx: number }> = [];

    for (const action of actions) {
        if (!action.target_text) continue;
        const lower = text.toLowerCase();
        const needle = action.target_text.toLowerCase();
        const pos = lower.indexOf(needle);
        if (pos === -1) continue;
        const idx = allActions.indexOf(action);
        matches.push({ start: pos, end: pos + action.target_text.length, action, idx });
    }
    ...
    for (const match of matches) {
        ...
        const isActive = activeIdx === match.idx;
        const type = match.action.action_type;

        let bg = 'rgba(245, 158, 11, 0.18)';
        let border = '2px solid rgba(245, 158, 11, 0.7)';
        ...
        nodes.push(
            <span
                key={`hl-${match.start}-${match.idx}`}
                style={{ background: bg, borderBottom: border, borderRadius: 2, padding: '1px 1px', cursor: 'pointer', ...extra }}
            >
...
// Render `text` with action highlights. Matching is whitespace/dot-insensitive
// so a target_text quote lands even when source spacing differs.
function highlightLine(
    ...
    for (const match of matches) {
        ...
        if (isActive) {
            bg = type === 'delete' ? 'rgba(239, 68, 68, 0.22)'
                : type === 'replace' ? 'rgba(59, 130, 246, 0.28)'
                    : 'rgba(245, 158, 11, 0.32)';
        }
        nodes.push(
            <span
...
function ResumeDocument({ text, filename, allActions, activeIdx, matchedIndices }: {
    ...
    return (
        <>
            {filename && (
                <div style={{ fontSize: 11, color: 'var(--accent)', background: 'var(--accent-soft)', border: '1px solid var(--accent)', borderRadius: 'var(--radius-sm)', padding: '3px 8px', display: 'inline-flex', alignItems: 'center', gap: 5, marginBottom: 14 }}>
            ...
            {lines.map((line, i) => {
                ...
                if (isMainSection(line)) {
                    ...
                    return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: i === 0 ? 0 : 20, marginBottom: 9 }}>
                            <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent)', flexShrink: 0 }}>
                                {label}
                            </span>
...
function ResumeMappingView({ actions, profile }: {
            ...
            <div style={{ padding: '24px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' }}>
                ...
                <a
                    ...
                    style={{
                        ...
                        marginTop: 4, height: 32, padding: '0 14px',
    ...
    function handleCardClick(i: number) {
        setSelectedIdx(i);
        const ref = sectionRefs[allActions[i].target_section];
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: '55% 45%', gap: 16, paddingTop: 12, alignItems: 'start' }}>
            {/* Left — paper resume */}
            <div style={{ maxHeight: 600, overflowY: 'auto', background: '#ffffff', borderRadius: 4, boxShadow: '0 1px 4px rgba(0,0,0,0.10), 0 0 0 1px rgba(0,0,0,0.06)', padding: '18px 20px', ...paperBase }}>
                {selectedResumeText ? (
                    <ResumeDocument
                        text={selectedResumeText}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 200, gap: 10, color: '#9ca3af' }}>
                        ...
                        <span style={{ fontSize: 13, textAlign: 'center', maxWidth: 240 }}>
                            Profile data unavailable — upload your resume to see the resume map
                        </span>
                    </div>
                ) : (
                    <>
                        {selectedResumeFilename && (
                            <div style={{ fontSize: 11, color: 'var(--accent)', background: 'var(--accent-soft)', border: '1px solid var(--accent)', borderRadius: 'var(--radius-sm)', padding: '3px 8px', display: 'inline-flex', alignItems: 'center', gap: 5, marginBottom: 10 }}>
                                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                {selectedResumeFilename}
                            </div>
                        )}
                        ...
                        <div ref={contactRef} style={paperSection}>
                        ...
                        {basics?.summary && (
                            <div ref={summaryRef} style={paperSection}>
                                <div style={paperHeading}>Summary</div>
                                <p style={{ margin: 0, fontSize: 12.5, color: '#374151', lineHeight: 1.6 }}>
                                    {highlightText(basics.summary, allActions, selectedIdx, allActions)}
                                </p>
                            </div>
                        )}

                        {(profile.work_experience?.length ?? 0) > 0 && (
                            <div ref={experienceRef} style={paperSection}>
                                    ...
                                    {profile.work_experience!.map((exp, i) => (
                                        <div key={i}>
                                            ...
                                            {[...(exp.description ?? []), ...(exp.achievements ?? [])].map((line, j) => (
                                                <div key={j} style={{ fontSize: 12, color: '#374151', lineHeight: 1.5, marginBottom: 2, paddingLeft: 10, position: 'relative' }}>
                                                    <span style={{ position: 'absolute', left: 0, color: '#9ca3af' }}>•</span>
                                                    {highlightText(line, allActions, selectedIdx, allActions)}
                                                </div>
                                            ))}
                                        </div>
                                    ))}
                        ...
                        {(profile.skills?.length ?? 0) > 0 && (
                            <div ref={skillsRef} style={paperSection}>
                                <div style={paperHeading}>Skills</div>
                                <div style={{ fontSize: 12.5, color: '#374151', lineHeight: 1.7 }}>
                                    {profile.skills!.map((skill, i) => (
                                        <span key={i}>
                                            {highlightText(skill, allActions, selectedIdx, allActions)}
                        ...
                        {(profile.education?.length ?? 0) > 0 && (
                            <div ref={educationRef} style={paperSection}>
            ...
            <div style={{ maxHeight: 600, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', color: 'var(--text-3)', textTransform: 'uppercase', marginBottom: 4, paddingLeft: 2 }}>
                    Changes ({allActions.length})
                </div>
