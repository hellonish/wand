...
// ── Developer constants ───────────────────────────────────────────────────────
// Change these when deploying to a hosted environment.
const WAND_API_URL = 'http://localhost:8000';
const WAND_APP_URL = 'http://localhost:3000';
...
async function verifyBackendConnection() {
  ...
  if (!token) {
    ...
    desc.textContent = 'Paste your Wand auth token below to sync your applications.';
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      desc.textContent = 'The token was rejected by the server. Please copy a fresh token from your Wand profile.';
    }
  } catch {
    card.className = 'connection-card failed';
    title.textContent = 'Server unreachable';
    desc.textContent = `Could not connect to ${WAND_API_URL}. The Wand server may be offline.`;
  }
}
...
function bindEvents() {
  ...
  document.getElementById('clear-data').addEventListener('click', async () => {
    if (!confirm('Clear the local job cache? This only removes cached data from the extension — your Wand account is not affected.')) return;
...
async function testConnection() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      resultEl.textContent = '✗ Token rejected — copy a fresh one from Wand';
      resultEl.className = 'test-result error';
    }
  } catch {
...
async function syncTokenFromTab() {
  try {
    const urlObj = new URL(WAND_APP_URL);
    const tabs = await chrome.tabs.query({});
    const targetTab = tabs.find(t => t.url && t.url.includes(urlObj.host));

    if (!targetTab) {
      showStatus(`Open ${WAND_APP_URL} and sign in, then try again.`, true);
      return;
    }
    ...
    if (result) {
      document.getElementById('wand-token').value = result;
      showStatus('✓ Token synced from Wand tab!');
    } else {
      showStatus('Sign in to Wand in the open tab first.', true);
    }
  } catch (err) {
    console.error('Sync failed:', err);
    showStatus('Failed to sync — make sure you\'re signed in to Wand.', true);
  }
}
